resource "azurerm_linux_function_app" "function_app" {
  ...
  identity {
    type         = "UserAssigned"
    identity_ids = [local.repository_funcapp_identity.id]
  }

  key_vault_reference_identity_id = local.repository_funcapp_identity.id
  ...
  app_settings = {
    ...
    "AzureAppConfiguration__ManagedIdentityClientId" = local.repository_funcapp_identity.client_id
    "AzureAppConfiguration__Environment"             = var.environment

    "AZURE_CLIENT_ID" = local.repository_funcapp_identity.client_id
