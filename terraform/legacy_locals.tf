locals {
  ...
  legacy_resource_group_name       = "rg-portal-repo-func-${var.environment}-${var.location}-${var.instance}"
  legacy_key_vault_name            = substr(format("kv-%s-%s", random_id.legacy_environment_id.hex, var.location), 0, 24)
  legacy_app_insights_name         = "ai-portal-repo-func-${var.environment}-${var.location}-${var.instance}"
  legacy_function_app_name         = "fn-portal-repo-func-${var.environment}-${var.location}-${var.instance}-${random_id.legacy_environment_id.hex}"
